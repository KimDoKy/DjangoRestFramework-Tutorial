...
Serializers는 querysets 및 모델 인스턴스와 같은 복잡한 데이터를 `JSON`, `XML` 또는 기타 컨텐트 유형으로 쉽게 렌더링 할 수 있는 Python 데이터 유형으로 변환해 줍니다. 또한 serializer는 deserialization을 제공하여, 들어오는 데이터의 유효성을 처음 확인한 후레 구문 분석 된 데이터를 복합 형식으로 다시 변환 할 수 있습니다.  
...
```
`comment`객체에 해당하는 데이터를 직렬화 및 비직렬화하는데 사용할 수 있는 serializer를 선언합니다.
...
### Serializing objects
`CommentSerializer`를 사용하여 주석 또는 주석 목록을 직렬화 할 수 있습니다. 다시 말하면, `Serializer`클래스를 사용하는 것은 `Form`클래스를 사용하는 것과 비슷합니다.
...
```
이 시점에서 모델 인스턴스를 파이썬 기본 데이터 유형으로 변환했습니다. 직렬화 과정을 마무리하기 위해 데이터를 `json`으로 렌더링합니다.
...
```
그런 다음 네이티브 데이터 유형을 검증 된 데이터 dict로 복원합니다.
...
def create(self, validated_data):
    ...
    def update(self, instance, validated_data):
        ...
        return instance
```
이제 테이터를 비직렬화 할 때 `.save()`를 호출하여 유효성이 검사된 데이터를 기반으로 객체 인스턴스를 반환 할 수 있습니다.
...
### Validation
데이터를 비직렬화 할 때 유효성이 검사 된 데이터에 액서스하기 전에 항상 `is_valid()`를 호출하거나 객체 인스턴스를 저장해야 합니다. 유효성 검사 오류가 발생하면, `.errors`속성에는 결과 오류 메세지를 나타내는 dict이 포함됩니다. 예를 들어:
...
항목 목록을 비직렬화 할 때 오류는 각 비직렬화 항목을 나타내는 dict 목록으로 반환됩니다.
...
#### Validators (검사기)
시리얼라이저의 개별 필드는 필드 인스턴스에 선언함으로써 유효성 검사기에 포함할 수 있습니다.
...
데이터의 비직렬화를 지원하는 중첩 된 표현을 처리할 때, 중첩 된 객체의 오류는 중첩 된 객체의 필드 이름 아래에 중첩됩니다.
...
Serializer 클래스는 객체 목록의 직렬화 또는 비직렬화를 처리 할 수도 있습니다.

#### 여러 객체 직렬화

단일 객체 인스턴스 대신 쿼리셋 또는 객체 목록을 직렬화하려면 serializer를 인스턴스화 할 때 `many=True`플래그를 전달해야 합니다. 그런 다름 직렬화 할 쿼리셋이나 객체 목록을 전달 할 수 있습니다.
...
#### 여러 객체를 비직렬화
여러 객체를 비직렬화하는 기본동작은 객체 생성을 지원하지만 여러 객체 업데이트를 지원하지 않습니다. 이러한 경우 중 하나를 지원하거나 사용자 지정하는 방법에 대한더 자세한 내용은 [ListSerializer](http://www.django-rest-framework.org/api-guide/serializers/#listserializer)를 참조하세요.

### Including extra context (추가 문맥 포함)
직렬화되고 있는 객체에 추가로, 직렬화에 여분의 문맥을 제공 할 필요가 이는 경우가 있습니다. 한 가지 일반적인 경우는 하이퍼링크 도니 관계를 포함하는 serializer를 사용하는 경우이며, serializer가 현재 요청에 액서스하여 정규화 된 URL을 제대로 생성 할 수 있어야합니다.  
...
기본 필드의 하위 집합을 모델 serializer에서만 사용하려는 경우 `ModelForm`에서와 마찬가지로 필드를 사용하거나 옵션을 제외할 수 있습니다. `fields`속성을 사용하여 직렬화해야하는 모든 필드를 명시적으로 설정하는 것이 좋습니다. 이렇게하면 모델이 변경 될 때 실수로 데이터가 노출 될 가능성이 줄어 듭니다.
...
### Specifying nested serialization (중첩 된 직렬화 지정)
...
직렬화가 수행되는 방식을 사용자 정의하려면 필드를 직접 정의해야합니다.
...
```
### Relational fields
모델 인스턴스를 직렬화 할 때 관계를 나타내기 위해 선택할 수 있는 여러 방법들이 있습니다. `ModelSerializer`의 기본 표현은 관련 인스턴스의 기본 키를 사용하는 것입니다.  
다른 표현은 하이퍼링크를 사용하여 직렬화, 완전한 중첩 표현을 직렬화,사용자 정의 표현을 사용하여 직렬화하는 것을 포함합니다.  
...
이 맵핑을 겹쳐 쓰면 각 모델 클래스에 사용해랴 하는 기본 직렬 serializer 클래스를 변경 할 수 있습니다.
...
- `.is_valid()` - 들어오는 데이터를 직렬화 해제와 검증합니다.
...
- `.to_representation()` - 읽기 조작을 위해 직렬화를 지원하려면 이를 오버라이드하세요.
...
```
이제 이 클래스를 사용하여 단일 `HighScore` 인스턴스를 직렬화 할 수 있습니다.
...
```
또는 이를 사용하여 여러 인스턴스를 직렬화할 수 있습니다.
...
`BaseSerializer` 클래스는 특정 직렬화 스타일을 처리하거나 대체 저장 장치 백엔드와 통합하기 위해 새 generic serializer 클래스를 구현하려는 경우에도 유용합니다.  
...
- 많은 양의 데이터를 반환하며 자주 액서스되는 API 엔드포인트의 직렬화 성능 향상
...
```
직렬화가 필요한 객체 인스턴스를 가져와서 원시 표현을 반환해야합니다. 일반적으로 이것은 내장 파이썬 데이터 유형의 구조를 반환하는 것을 의미합니다. 처리 할 수 있는 정확한 유형은 API에 대해 구성한 렌더링 클래스에 따라 다릅니다.
